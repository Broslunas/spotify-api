...
app.get("/spotify/login", (req, res) => {
  const scope =
    "user-read-private user-read-email user-top-read user-read-currently-playing user-read-recently-played user-read-playback-state user-modify-playback-state playlist-read-private playlist-read-collaborative";
  res.redirect(
...
app.put("/spotify/play-playlist", async (req, res) => {
  ...
  try {
    ...
    console.error(
      "Error al reproducir la playlist:",
      error.response ? error.response.data : error
    );
    res
      .status(error.response ? error.response.status : 500)
